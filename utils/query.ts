...
export async function fetchTrending(): Promise<{ data: TrendingToken2[] }> {
  return apiRequest<{ data: TrendingToken2[] }>(`${config.apiEndpoint}/trending`);
}
...
export async function fetchTokenDetails(tokenId: string, chain: 'SOL' | 'ETH'): Promise<{ data: TrendingToken2 }> {
  return apiRequest<{ data: TrendingToken2 }>(`${config.apiEndpoint}/token/${chain.toLowerCase()}/${tokenId}`);
}

export async function fetchMarketData(timeframe: '24h' | '7d' | '30d' = '24h'): Promise<APIResponse<any>> {
  return apiRequest<APIResponse<any>>(`${config.apiEndpoint}/market?timeframe=${timeframe}`);
}
